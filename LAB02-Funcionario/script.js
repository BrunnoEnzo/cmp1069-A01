class Funcionario {
    ...
    toString() {
        return `Nome: ${this.getNome()}, Idade: ${this.getIdade()}, Cargo: ${this.getCargo()}, Salário: ${this.getSalario()}`;
    }
}
...
const cadastrarFuncionario = () => {
    ...
    for (let i = 0; i < funcionarios.length; i++) {
        if (funcionarios[i].getNome() === nome && funcionarios[i].getCargo() === cargo && i !== indiceEdicao) {
...
document.querySelector('.relatorios button:nth-child(2)').addEventListener('click', () => {
    if (funcionarios.length === 0) {
        mostrarResultado('Ganham acime de R$5000', 'Nenhum funcionário cadastrado');
        return;
    }
...
document.querySelector('.relatorios button:nth-child(4)').addEventListener('click', () => {
    if (funcionarios.length === 0) {
        mostrarResultado('Cargos únicos', 'Nenhum funcionário cadastrado');
        return;
    }
...
document.querySelector('.relatorios button:nth-child(5)').addEventListener('click', () => {
    if (funcionarios.length === 0) {
        mostrarResultado('Lista de nomes em maiúsculo', 'Nenhum funcionário cadastrado');
        return;
    }
